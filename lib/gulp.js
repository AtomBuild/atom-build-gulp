...
export function provideBuilder() {
  return class GulpBuildProvider extends EventEmitter {
    ...
    isEligible() {
      this.files = [ 'gulpfile.js', 'gulpfile.coffee', 'gulpfile.babel.js' ]
    ...
    settings() {
      ...
      return new Promise((resolve, reject) => {
        ...
        Task.once(require.resolve(__dirname + '/parser-task.js'), this.cwd, (result) => {
          if (result.error) {
          ...
          this.fileWatchers.forEach(fw => fw.close());
          this.fileWatchers = this.files.map(file => fs.watch(file, () => {
